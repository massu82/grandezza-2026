        ...
        <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($pages ?? [] as $page)
                    <tr>
                        <td class="px-4 py-3 text-sm font-medium text-slate-900">{{ $page->titulo }}</td>
                        <td class="px-4 py-3 text-sm text-slate-600">{{ $page->slug }}</td>
                        <td class="px-4 py-3 text-sm">
                            @if($page->publicado)
                                <span class="px-2 py-1 text-xs rounded-full bg-emerald-50 text-emerald-700">Publicado</span>
                            @else
                                <span class="px-2 py-1 text-xs rounded-full bg-slate-100 text-slate-600">Borrador</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm text-right space-x-2">
                            <a href="{{ url('/admin/pages/'.$page->id.'/edit') }}" class="text-slate-700 hover:underline">Editar</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-6 text-center text-sm text-slate-500">No hay páginas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
