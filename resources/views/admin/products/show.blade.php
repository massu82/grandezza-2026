<x-layout-admin :title="$product->nombre">
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-4">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-rose-950" style="font-family: 'Playfair Display', serif;">{{ $product->nombre }}</h1>
                <p class="text-sm text-slate-600">SKU: {{ $product->sku }}</p>
            </div>
            <a href="{{ url('/admin/products/'.$product->id.'/edit') }}" class="text-sm text-rose-900 hover:underline">Editar</a>
        </div>
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex items-center justify-between"><span class="text-slate-500">Precio:</span><span class="font-semibold text-rose-900">${{ number_format($product->precio, 2) }}</span></div>
                @if($product->precio_promocion)
                    <div class="flex items-center justify-between"><span class="text-slate-500">Promoción:</span><span class="font-semibold text-rose-900">${{ number_format($product->precio_promocion, 2) }}</span></div>
                @endif
