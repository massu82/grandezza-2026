<x-layout-admin :title="$promotion->titulo">
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-4">
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-rose-950" style="font-family: 'Playfair Display', serif;">{{ $promotion->titulo }}</h1>
                <p class="text-sm text-slate-600">{{ $promotion->descripcion }}</p>
            </div>
            <a href="{{ url('/admin/promotions/'.$promotion->id.'/edit') }}" class="text-sm text-rose-900 hover:underline">Editar</a>
        </div>
