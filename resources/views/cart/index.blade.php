<x-layout-public title="Carrito | Grandezza">
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-3xl font-semibold text-primary" style="font-family: 'Playfair Display', serif;">Tu carrito</h1>
            <a href="{{ url('/vinos') }}" class="text-sm text-primary hover:text-secondary">Seguir comprando</a>
        </div>
                ...
                @forelse(($cartItems ?? []) as $item)
                    <div class="p-4 grid grid-cols-1 md:grid-cols-[1fr_auto_auto] items-center gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="text-sm font-semibold text-primary">{{ $item['nombre'] ?? $item['name'] ?? 'Producto' }}</div>
                                <div class="text-xs text-zinc-500">{{ $item['categoria'] ?? $item['options']['categoria'] ?? '' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="text-left md:text-right md:min-w-[140px]">
                            ...
                            <div class="text-lg font-semibold text-primary">${{ number_format($item['subtotal'] ?? ($item['price'] ?? $item['precio'] ?? 0), 2) }}</div>
                        </div>
                    </div>
                @empty
                    <div class="p-6 text-center text-zinc-500 text-sm">No tienes productos en tu carrito.</div>
                @endforelse
            </div>
        </div>
        ...
        @if(isset($cartItems) && count($cartItems))
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="text-sm text-zinc-600">Total: <span class="text-xl font-semibold text-primary">${{ number_format($cartTotal ?? 0, 2) }}</span></div>
                <a href="{{ url('/checkout') }}" class="btn-primary" data-gtm-event="begin_checkout" data-meta-event="InitiateCheckout">Continuar al checkout</a>
            </div>
        @endif
