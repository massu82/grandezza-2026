    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-4">
                ...
                <p class="text-sm text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-4 py-2">Pago seguro con Stripe. Entrega por recolección en tienda.</p>
                <form method="POST" action="{{ url('/checkout/stripe') }}" class="space-y-4" x-data="formState()" @submit="start($event)">
                    @csrf
                    ...
                    <div class="flex items-center gap-3">
                        <x-button-primary type="submit" data-gtm-event="purchase" data-meta-event="Purchase" x-bind:disabled="submitting">
                            <span x-show="!submitting">Pagar con tarjeta (Stripe)</span>
                            <span x-show="submitting">Redirigiendo a pago...</span>
                        </x-button-primary>
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
                <h2 class="text-lg font-semibold text-primary mb-3" style="font-family: 'Playfair Display', serif;">Resumen</h2>
                <div class="space-y-4">
                    @forelse(($cartItems ?? []) as $item)
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-sm font-semibold text-primary">${{ number_format($item->subtotal ?? ($item->price ?? 0), 2) }}</div>
                        </div>
                    @empty
                    ...
                    <div class="border-t border-slate-200 pt-4 flex items-center justify-between">
                        ...
                        <span class="text-xl font-semibold text-primary">${{ number_format($cartTotal ?? 0, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
