<x-layout-public title="Bolsa de trabajo | Grandezza">
    <section class="w-full bg-slate-900 text-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <p class="text-sm text-slate-200 mb-2">Buscamos personas apasionadas por el vino, el servicio y la hospitalidad.</p>
        </div>
    </section>
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-6">
        <div class="grid md:grid-cols-2 gap-8">
            <div>
                <p class="text-sm text-slate-600 mb-4">Comparte tus datos y cuéntanos por qué quieres trabajar con nosotros.</p>
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
                <form method="POST" action="{{ url('/bolsa') }}" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="file" id="cv" name="cv" accept="application/pdf" class="block w-full text-sm text-slate-700 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-primary file:text-white hover:file:bg-secondary border border-slate-300 rounded-lg focus:border-primary focus:ring-accent">
                        @error('cv')
