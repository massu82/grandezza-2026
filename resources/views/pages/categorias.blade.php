<x-layout-public title="Categorías | Grandezza">
    <section class="w-full bg-gradient-to-b from-zinc-900 via-zinc-950 to-zinc-900 text-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <p class="text-sm text-zinc-200 mt-2">Explora nuestras familias de vinos y destilados.</p>
        </div>
    </section>
            ...
            @forelse($categories ?? [] as $category)
                <a href="{{ url('/categorias/'.$category->slug) }}" class="block bg-white border border-zinc-200 rounded-xl shadow-sm hover:shadow-md transition overflow-hidden">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm text-zinc-600 mt-1">{{ \Illuminate\Support\Str::limit($category->descripcion, 80) }}</p>
                    </div>
                </a>
            @empty
                <p class="col-span-full text-sm text-zinc-500">No hay categorías disponibles.</p>
            @endforelse
        </div>
        @if(isset($categories))
