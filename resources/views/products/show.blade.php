<x-layout-public :title="$product->nombre ?? 'Vino'" :meta-description="$product->descripcion_corta ?? ''">
    <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <nav class="text-xs text-slate-500 mb-4">
            <a href="{{ url('/') }}" class="hover:text-rose-900">Home</a>
            <span class="mx-2">/</span>
            @if($product->category ?? false)
                <a href="{{ url('/categorias/'.$product->category->slug) }}" class="hover:text-rose-900">{{ $product->category->nombre }}</a>
                <span class="mx-2">/</span>
            @endif
            <span class="text-rose-900">{{ $product->nombre }}</span>
        </nav>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                <h1 class="text-3xl font-semibold text-rose-950" style="font-family: 'Playfair Display', serif;">{{ $product->nombre }}</h1>
                <p class="text-sm text-slate-600">{{ $product->descripcion_corta }}</p>
                <div class="flex flex-wrap gap-3 text-sm text-slate-600">
                    @if($product->tipo)<span class="px-3 py-1 bg-rose-50 text-rose-900 rounded-full">{{ $product->tipo }}</span>@endif
                ...
                <div class="flex items-center gap-3">
                    @if($product->precio_promocion)
                        <span class="text-3xl font-semibold text-rose-900">${{ number_format($product->precio_promocion, 2) }}</span>
                        <span class="text-lg text-slate-400 line-through">${{ number_format($product->precio, 2) }}</span>
                    @else
                        <span class="text-3xl font-semibold text-rose-900">${{ number_format($product->precio, 2) }}</span>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2 text-slate-600 text-sm ml-auto">
                        <span>Compartir:</span>
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank" rel="noopener" aria-label="Compartir en Facebook" class="p-1 rounded hover:text-rose-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M13.5 9H16V6h-2.5a3.5 3.5 0 0 0-3.5 3.5V12H8v3h2v6h3v-6h2.1l.4-3H13V9.5c0-.3.2-.5.5-.5Z"/>
                            </svg>
                        </a>
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($product->nombre) }}" target="_blank" rel="noopener" aria-label="Compartir en X" class="p-1 rounded hover:text-rose-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M5 4h3l4 5.5L16.5 4H19l-5.4 7.3L19 20h-3l-4.4-5.8L7.3 20H5l5.6-7.5L5 4Z"/>
                            </svg>
                        </a>
                        <a href="https://api.whatsapp.com/send?text={{ urlencode($product->nombre.' '.url()->current()) }}" target="_blank" rel="noopener" aria-label="Compartir en WhatsApp" class="p-1 rounded hover:text-rose-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
    ...
    @if(($related ?? null) && $related->isNotEmpty())
        <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <p class="text-xs uppercase tracking-[0.2em] text-rose-700">Te puede interesar</p>
                    <h2 class="text-2xl font-semibold text-rose-950" style="font-family: 'Playfair Display', serif;">Vinos relacionados</h2>
                </div>
